...
function init_review_map() {
      ...
      assessment_features = new ol.source.Vector({wrapX: false});
...
function init_overview_map() {
	if ( USE_LEAFLET == true ) {
      ...
      overview_features = new ol.source.Vector({wrapX: false});

      var vector = new ol.layer.Vector({
        source: overview_features
      });
      
		var osm = new ol.layer.Tile({ source: new ol.source.OSM(), zIndex: -1, visible: false});
...
function set_overview_image(image) {
	...
	if ( USE_LEAFLET == true ) {
		overview_map.setView([parseFloat(image["Latitude"]), parseFloat(image["Longitude"])], OVR_ZOOM);
	} else {
	   var vw = overview_map.getView();
	   var newPt = ol.proj.transform([parseFloat(image["Longitude"]), parseFloat(image["Latitude"])], 'EPSG:4326','EPSG:3857');
	   vw.setCenter(newPt);
	   if ( vw.getZoom() < 12 ) {
	   	vw.setZoom(OVR_ZOOM);   
		}
...
function addMarkerTool(cls) {
	...
	draw_tool.on('drawend', function(e) {
	  ...
	  e.feature.setProperties({"dmgClass": myCls}, true);
	});
